...
from spaceone.dashboard.info import (
...
from test.factory.project_dashboard_version_factory import (
...
class TestProjectDashboardService(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.transaction = Transaction(
            {"service": "dashboard", "api_class": "ProjectDashboard"}
        )
        super().setUpClass()
    ...
    @parameterized.expand(
        [["user_id", None], ["user_id", "cloudforet@gmail.com"]],
    ...
    def test_create_project_dashboard(self, key, value):
        params = {
            ...
            "variables": {"project_id": "project-1234"},
            "settings": {
    ...
    def test_update_project_dashboard_permission_error(self):
        project_dashboard_vo = ProjectDashboardFactory(
            domain_id=self.domain_id, viewers="PRIVATE", user_id="cloudforet2@gmail.com"
        )
    ...
    def test_delete_project_dashboard(self):
        project_dashboard_vo = ProjectDashboardFactory(domain_id=self.domain_id)
        project_dashboard_version_vos = ProjectDashboardVersionFactory.build_batch(
            5,
            project_dashboard_id=project_dashboard_vo.project_dashboard_id,
            domain_id=self.domain_id,
        )
        ...
        for idx, project_dashboard_version in enumerate(
            project_dashboard_version_vos, 1
        ):
            print_data(
                project_dashboard_version.to_dict(),
                f"{idx}th project_dashboard_version",
            )
    ...
    def test_revert_version(self):
        ...
        self.assertEqual(
            project_dashboard_vo.variables_schema, first_version.variables_schema
        )
    ...
    def test_get_version(self):
        ...
        print_data(
            project_dashboard_version_vo.to_dict(),
            "test_project_dashboard_version_by_get_version",
        )
        ...
        self.assertEqual(
            project_dashboard_version_vo.project_dashboard_id,
            project_dashboard_vo.project_dashboard_id,
        )
        self.assertEqual(
            project_dashboard_version_vo.layouts, project_dashboard_vo.layouts
        )
        self.assertEqual(
            project_dashboard_version_vo.variables, project_dashboard_vo.variables
        )
        self.assertEqual(
            project_dashboard_version_vo.variables_schema,
    ...
    def test_list_versions(self):
        ...
        project_dashboard_svc = ProjectDashboardService(transaction=self.transaction)
        (
            list_project_dashboard_version_vos,
            total_count,
            current_version,
        ) = project_dashboard_svc.list_versions(params.copy())

        ProjectDashboardVersionsInfo(list_project_dashboard_version_vos, total_count)

        for idx, domain_dashboard_version in enumerate(
            list_project_dashboard_version_vos, 1
        ):
            print_data(
                domain_dashboard_version.to_dict(), f"{idx}th project_dashboard_version"
            )

        self.assertEqual(len(list_project_dashboard_version_vos), 5)
        self.assertIsInstance(
            list_project_dashboard_version_vos[0], ProjectDashboardVersion
        )
    ...
    def test_get_project_dashboard(self):
        ...
        self.assertEqual(
            project_dashboard_vo.project_dashboard_id,
            get_project_dashboard_vo.project_dashboard_id,
        )

    def test_list_project_dashboards(self):
        project_dashboard_vos = ProjectDashboardFactory.build_batch(
        ...
        self.assertEqual(
            list_project_dashboard_vos[0].project_dashboard_id,
            params.get("project_dashboard_id"),
        )
