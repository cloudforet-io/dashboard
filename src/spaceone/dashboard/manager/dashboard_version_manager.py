...
class DashboardVersionManager(BaseManager):
    def __init__(self, *args, **kwargs):
        ...
        self.domain_dashboard_version_model: DashboardVersion = self.locator.get_model(
            "DashboardVersion"
        )
    ...
    def create_version_by_domain_dashboard_vo(self, domain_dashboard_vo, params):
        def _rollback(version_vo):
            _LOGGER.info(
                f"[create_domain_dashboard_version._rollback] "
        ...
        new_params = {
            ...
            else domain_dashboard_vo.layouts,
            "variables": params.get("variables")
            if params.get("variables")
            else domain_dashboard_vo.variables,
            "settings": params.get("settings")
            if params.get("settings")
            else domain_dashboard_vo.settings,
            "variables_schema": params.get("variables_schema")
            if params.get("variables_schema")
            else domain_dashboard_vo.variables_schema,
            "domain_id": domain_dashboard_vo.domain_id,
        }

        version_vo: DashboardVersion = self.domain_dashboard_version_model.create(
            new_params
        )
    ...
    def delete_version(self, domain_dashboard_id, version, domain_id):
        version_vo: DashboardVersion = self.get_version(
            domain_dashboard_id, version, domain_id
        )
        version_vo.delete()

    def delete_versions_by_domain_dashboard_version_vos(
        self, domain_dashboard_version_vos
    ):
        domain_dashboard_version_vos.delete()

    def get_version(self, domain_dashboard_id, version, domain_id, only=None):
        return self.domain_dashboard_version_model.get(
            domain_dashboard_id=domain_dashboard_id,
            version=version,
            domain_id=domain_id,
            only=only,
        )
